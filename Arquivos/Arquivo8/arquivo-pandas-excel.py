...
def gravar_arquivo (arq, df_turma, df_aprovacao):
    try:
        with pd.ExcelWriter(arq) as writer:
            ...
            df_aprovacao.to_excel(writer, sheet_name="aprovação", index=False)
        print("dados gravados com sucesso")
    except:
        print("Erro ao gravar o arquivo")


arq = definir_arquivo("turma.xlsx")
df_turma = ler_arquivo(arq)
# print(df_turma)
df_aprovacao =  verificar_aprovacao(df_turma)
print(df_aprovacao)
arq = definir_arquivo("turma2.xlsx")
gravar_arquivo(arq, df_turma, df_aprovacao)